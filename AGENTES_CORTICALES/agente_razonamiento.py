...
class AgenteRazonamiento(BaseAgente):
    ...
    def _aprender(self, texto: str, hecho) -> dict[str, Any]:
        ...
        if self.neurogenesis is not None:
            self.neurogenesis.crecimiento.crecer_desde_aprendizaje(
                agregado.sujeto, agregado.relacion, agregado.objeto
            )
    ...
    def _responder_pregunta(self, pregunta: str, estrategias: list[str] | None = None) -> dict[str, Any]:
        ...
        if self.neurogenesis is not None and getattr(inferencia, "hechos_usados", None):
            for h in inferencia.hechos_usados:
                self.neurogenesis.registrar_hecho_usado(h.sujeto, h.relacion, h.objeto)
            # Crecimiento: materializar conclusión deductiva transitiva como hecho inferido
            if inferencia.estrategia == "deductiva" and inferencia.confianza >= 0.55:
                entidad = normalizar(analisis.entidad_principal) if analisis.entidad_principal else None
                if entidad and inferencia.hechos_usados:
                    ultimo = inferencia.hechos_usados[-1]
                    self.neurogenesis.crecimiento.crecer_desde_inferencia(
                        self.grafo,
                        entidad,
                        ultimo.relacion,
                        ultimo.objeto,
    ...
    def _razonar(self, analisis, estrategias: list[str] | None = None):
        ...
        # Preguntas de función/propiedad: priorizar tiene_funcion / tiene_propiedad
        if analisis.intencion == "propiedad" and entidad and self.grafo.existe(entidad):
            for rel in ("tiene_funcion", "tiene_propiedad", "tiene_proposito", "tiene"):
                res = self.motor.deducir(entidad, relacion_objetivo=rel)
                if res.conclusion:
                    return res
