...
def test_extrae_sirve_para():
    from AGENTES_CORTICALES.razonamiento.extractor_hechos import extraer_hecho
    h = extraer_hecho("la metacognicion sirve para evaluar el razonamiento")
    assert h is not None
    assert h.relacion == "tiene_funcion"


def test_extrae_pertenece_a():
    from AGENTES_CORTICALES.razonamiento.extractor_hechos import extraer_hecho
    h = extraer_hecho("el agente emocional pertenece a los agentes corticales")
    assert h is not None
    assert h.relacion == "es_parte_de"
