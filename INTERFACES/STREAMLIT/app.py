...
def main() -> None:
    ...
    with st.sidebar:
        ...
        up = st.file_uploader(
            "Imagen o audio",
            type=["png", "jpg", "jpeg", "webp", "gif", "wav", "mp3", "ogg", "txt"],
            help="Opcional: añadí una descripción abajo para razonar sobre el archivo",
        )
        caption = st.text_input("Descripción / transcripción manual", "")
        if up is not None and st.button("Enviar archivo", use_container_width=True):
            ...
            perc = norm.desde_upload(up.name, data, caption=caption or None, guardar_en=dest)
    ...
    if "pending_multimodal" not in st.session_state:
        st.session_state.pending_multimodal = None

    user_text = None
    multimodal = None
    if st.session_state.pending_multimodal:
        multimodal = st.session_state.pending_multimodal
        st.session_state.pending_multimodal = None
        user_text = multimodal.get("texto_para_razonar") or multimodal.get("contenido") or "[multimodal]"
    elif st.session_state.pending_cmd:
